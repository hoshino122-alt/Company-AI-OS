...
define gui.accent_color = '#00ffe1'

## selected（選択中）でも hover（フォーカス中）でもない状態のテキストボタンのカ
## ラー。
define gui.idle_color = '#00cccc'
...
## のボタンが用意されています。radio と check は環境設定の各項目のボタン（デフォ
## ルトでは同じ画像）。confirm は確認画面の選択肢、page は セーブ・ロード画面の
## ページ切り替え、quick はクイックメニュー、 navigation はゲームメニューのメニ
## ュー切り替えに使うボタンです。
